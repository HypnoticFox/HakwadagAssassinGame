...
test.describe('Administration', () => {
  ...
  async function openAdminPage(page: Page) {
    ...
    await expect(page.getByRole('heading', { name: 'Spelinstellingen' })).toBeVisible()
  }
  ...
  test('admin page is accessible for game creator', async ({ page }) => {
    ...
    const adminBtn = page.getByRole('button', { name: 'Spelinstellingen' })
  ...
  test('admin page shows settings, conditions, safe time, and moderators sections', async ({
    ...
    await expect(page.getByRole('heading', { name: 'Voorwaarden' })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Veilige tijdvakken' })).toBeVisible()
  ...
  test('add safe time block: admin adds block → shown on game detail', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Veilige tijd toevoegen' }).click()

    // Verify the API call was made by checking the form is still usable
    await expect(page.getByRole('button', { name: 'Veilige tijd toevoegen' })).toBeVisible()
  })
  ...
  test('add custom condition: admin adds condition → input is cleared', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Voorwaarde toevoegen' }).click()
  ...
  test('update confirmation timeout and assignment cooldown', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Opslaan', exact: true }).first()).toBeEnabled()

    await numberInputs.nth(1).fill('15')
    await page.getByRole('button', { name: 'Opslaan', exact: true }).nth(1).click()
    await expect(page.getByRole('button', { name: 'Opslaan', exact: true }).nth(1)).toBeEnabled()
  })

  test('back to game button returns to the game detail page', async ({ page }) => {
    await openAdminPage(page)

    await page.getByRole('button', { name: 'Terug naar spel' }).click()
